	...
	Properties
	{
		...
		SubShader
		{
			...
			CGPROGRAM
				...
				float borderval = clamp(smoothstep(0.4, 0.5, abs(0.5 - (IN.texcoordW.x - float2(0, _Time.y)))), 0, 1);

				float2 the = step(borderval, 0.01) * ( float2(-0.09 * _Time.y, -0.03 * _Time.y) + tex2D(_DistortTex, IN.texcoordD).r * float2(0.6, 0.3) );
				//IN.texcoordW += the;



					//Glitch Effect I discovered!
					//float distortion = tex2D(_DistortTex, IN.texcoordD + float2(_Time.y * 0.3, 0)) * IN.color; //0 - 1
					...
					//float distortion = tex2D(_DistortTex, IN.texcoordD + float2(_Time.y * 0.3, 0)) * IN.color; //0 - 1
					//distortion -= 0.5; //-0.5 - 0.5
					//distortion *= 0.1; //-0.25 + 0.25
				float windValue = tex2D(_WindTex, IN.texcoordW - float2(0, _Time.y) + the).r; //+ distortion;
					clip(step(0.01, borderval) + windValue - _WindThreshold);
					c.a *= smoothstep(_WindThreshold, 1.0, windValue);
					float theval = 0.15 * (1 - pow(clamp(smoothstep(0.0, 0.8, IN.texcoordW.y), 0, 1), 2));
					c.a *= 1 - clamp(smoothstep(0.4 - theval, 0.5 - theval, abs(0.5 - IN.texcoordW.x)), 0, 1);
					c.a *= 1 - smoothstep(0.01, 0.1, abs(abs(IN.texcoordW.y - _Time.y) % 0.2 - 0.1));

					c.a += borderval;

					c.a *= 1 - clamp(smoothstep(0.8, 1, IN.texcoordW.y), 0, 1);
