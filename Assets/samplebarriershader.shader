Shader "Unlit/samplebarriershader"
{
    Properties
    {
        [PerRendererData] _MainTex("Sprite Texture", 2D) = "white" {}
        _Color("Tint", Color) = (1,1,1,1)
        _Strawberry("Strawberry", Color) = (1,1,1,1)
        [MaterialToggle] PixelSnap("Pixel snap", Float) = 0
        _WindTex("Wind Texture", 2D) = "white" {}
        _WindThreshold("Wind Threshold", Range(0, 1)) = 0.4
        _DistortTex("Distort Texture", 2D) = "white" {}
        _IsOpen("Is Open", Range(0, 1)) = 0
    }
        SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            float thresholdB = 1;
            c.a *= 0.4 * (1 - clamp(smoothstep(thresholdA - 0.2, thresholdA + 0.05, IN.texcoord.y + 0.5 * pow(abs(0.5 - IN.texcoord.x), 2.5)), 0, 1))
                + 0.2 * (1 - clamp(smoothstep(thresholdB - 0.2, thresholdB + 0.05, IN.texcoord.y + 0.5 * pow(abs(0.5 - IN.texcoord.x), 3)), 0, 1))
                + 0.4 * (1 - clamp(smoothstep(thresholdA - 0.2, thresholdA + 0.05, 1 - IN.texcoord.y + 0.5 * pow(abs(0.5 - IN.texcoord.x), 2.5)), 0, 1))
                ;
            c.a *= (1 +2 * pow(clamp(smoothstep(0.4, 0.5, abs(0.5 - IN.texcoord.x)), 0, 1), 1.4));
            c.a *= (clamp(smoothstep(0, 0.01, IN.texcoord.y), 0, 1));
            float thecond = clamp(_IsOpen + 1 - clamp(smoothstep(0.0, 0.3, abs(0.5 - IN.texcoord.x)), 0, 1), 0, 1);
            c.rgb = _Strawberry.rgb * (1 - thecond) + c.rgb * thecond;
            ...
            float2 windCoord = IN.texcoordW - float2(0, _Time.y * 0.1);
            ...
            contentC.a *= (1 - clamp(smoothstep(0.4, 0.49, abs(0.5 - ((abs(IN.texcoord.y - 0.1 * 0.49 * _Time.y) * 2.04 - 0.02) % 1))), 0, 1))
                ;
